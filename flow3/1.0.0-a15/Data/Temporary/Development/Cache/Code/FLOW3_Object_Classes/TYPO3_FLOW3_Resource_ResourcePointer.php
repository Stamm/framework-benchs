 ...
 * @valueobject 
 */
class ResourcePointer extends ResourcePointer_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface, \TYPO3\FLOW3\Persistence\Aspect\PersistenceMagicInterface {
	...
	public function __construct() {
		...
		if (!isset($arguments[0])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $hash in class ' . __CLASS__ . '. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.', 1296143788);
		call_user_func_array('parent::__construct', $arguments);
				...
				if (array_key_exists(0, $arguments)) $methodArguments['hash'] = $arguments[0];
			
					$advices = $this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices['__construct']['TYPO3\FLOW3\AOP\Advice\BeforeAdvice'];
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Resource\ResourcePointer', '__construct', $methodArguments);
		...
		$this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices = array(
			'__construct' => array(
				'TYPO3\FLOW3\AOP\Advice\BeforeAdvice' => array(
					new \TYPO3\FLOW3\AOP\Advice\BeforeAdvice('TYPO3\FLOW3\Persistence\Aspect\PersistenceMagicAspect', 'generateValueHash', $objectManager, NULL),
				),
			),
			'__clone' => array(
	 ...
	 public function __clone() {
		...
		$result = NULL;
			...
			try {
					...
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Resource\ResourcePointer', '__clone', $methodArguments);
					$result = $this->FLOW3_AOP_Proxy_invokeJoinPoint($joinPoint);

					$advices = $this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices['__clone']['TYPO3\FLOW3\AOP\Advice\AfterReturningAdvice'];
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Resource\ResourcePointer', '__clone', $methodArguments, NULL, $result);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}
	...
	$reflectedClass = new \ReflectionClass('TYPO3\FLOW3\Resource\ResourcePointer');
	...
	foreach($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyTaggedWith('TYPO3\FLOW3\Resource\ResourcePointer', $propertyName, 'transient')) continue;
...
#0             %CLASS%TYPO3_FLOW3_Resource_ResourcePointer %CLASS%_TYPO3_FLOW3_Persistence_Aspect_PersistenceMagicInterface8621      