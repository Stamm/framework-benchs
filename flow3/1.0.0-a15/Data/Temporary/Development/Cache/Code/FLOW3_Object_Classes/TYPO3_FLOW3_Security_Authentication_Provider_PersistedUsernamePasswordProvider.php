 ...
 * @scope prototype
 */
class PersistedUsernamePasswordProvider extends PersistedUsernamePasswordProvider_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if (!isset($arguments[1])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $options in class ' . __CLASS__ . '. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.', 1296143788);
		...
		$result = parent::authenticate($authenticationToken);
			...
			try {
				...
				$methodArguments['authenticationToken'] = $authenticationToken;
					...
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Security\Authentication\Provider\PersistedUsernamePasswordProvider', 'authenticate', $methodArguments);
					...
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Security\Authentication\Provider\PersistedUsernamePasswordProvider', 'authenticate', $methodArguments, NULL, $result);
	...
	$reflectedClass = new \ReflectionClass('TYPO3\FLOW3\Security\Authentication\Provider\PersistedUsernamePasswordProvider');
	...
	foreach($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyTaggedWith('TYPO3\FLOW3\Security\Authentication\Provider\PersistedUsernamePasswordProvider', $propertyName, 'transient')) continue;
	 ...
	 */
	 private function FLOW3_Proxy_injectProperties() {
		$this->accountRepository = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Security\AccountRepository');
		$this->hashService = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Security\Cryptography\HashService');
	}
}
#0             %CLASS%TYPO3_FLOW3_Security_Authentication_Provider_PersistedUsernamePasswordProvider8840      