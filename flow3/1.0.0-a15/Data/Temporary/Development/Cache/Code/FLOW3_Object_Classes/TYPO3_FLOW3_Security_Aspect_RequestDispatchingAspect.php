 ...
 * @aspect 
 */
class RequestDispatchingAspect extends RequestDispatchingAspect_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		\TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\FLOW3\Security\Aspect\RequestDispatchingAspect', $this);

		if (!isset($arguments[0])) $arguments[0] = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Security\Context');
		if (!isset($arguments[1])) $arguments[1] = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Security\Authorization\FirewallInterface');
		if (!isset($arguments[2])) $arguments[2] = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Log\SecurityLoggerInterface');
		if (!isset($arguments[0])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $securityContext in class ' . __CLASS__ . '. Please check your calling code and Dependency Injection configuration.', 1296143787);
		if (!isset($arguments[1])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $firewall in class ' . __CLASS__ . '. Please check your calling code and Dependency Injection configuration.', 1296143787);
		if (!isset($arguments[2])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $securityLogger in class ' . __CLASS__ . '. Please check your calling code and Dependency Injection configuration.', 1296143787);
	...
	$reflectedClass = new \ReflectionClass('TYPO3\FLOW3\Security\Aspect\RequestDispatchingAspect');
	...
	foreach($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyTaggedWith('TYPO3\FLOW3\Security\Aspect\RequestDispatchingAspect', $propertyName, 'transient')) continue;
...
#0             %CLASS%TYPO3_FLOW3_Security_Aspect_RequestDispatchingAspect5322      