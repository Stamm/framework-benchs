 ...
 * @scope prototype
 */
class LogicalAnd extends LogicalAnd_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if (!isset($arguments[0])) $arguments[0] = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Persistence\Generic\Qom\Constraint');
		if (!isset($arguments[1])) $arguments[1] = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Persistence\Generic\Qom\Constraint');
		if (!isset($arguments[0])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $constraint1 in class ' . __CLASS__ . '. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.', 1296143788);
		if (!isset($arguments[1])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $constraint2 in class ' . __CLASS__ . '. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.', 1296143788);
	...
	$reflectedClass = new \ReflectionClass('TYPO3\FLOW3\Persistence\Generic\Qom\LogicalAnd');
	...
	foreach($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyTaggedWith('TYPO3\FLOW3\Persistence\Generic\Qom\LogicalAnd', $propertyName, 'transient')) continue;
...
#0             %CLASS%TYPO3_FLOW3_Persistence_Generic_Qom_LogicalAnd %CLASS%TYPO3_FLOW3_Persistence_Generic_Qom_Constraint4810      