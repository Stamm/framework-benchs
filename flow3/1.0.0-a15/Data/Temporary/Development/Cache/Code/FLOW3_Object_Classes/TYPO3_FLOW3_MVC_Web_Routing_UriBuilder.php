 ...
 * @scope prototype
 */
class UriBuilder extends UriBuilder_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface {
		...
		$this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices = array(
			'build' => array(
				'TYPO3\FLOW3\AOP\Advice\BeforeAdvice' => array(
					new \TYPO3\FLOW3\AOP\Advice\BeforeAdvice('TYPO3\FLOW3\Security\Aspect\CsrfProtectionAspect', 'addCsrfTokenToUri', $objectManager, NULL),
				),
			),
		);
	}
	 ...
	 */
	 public function build(array $arguments = array()) {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['build'])) {
		$result = parent::build($arguments);

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['build'] = TRUE;
			try {
				...
				$methodArguments['arguments'] = $arguments;
					...
					$advices = $this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices['build']['TYPO3\FLOW3\AOP\Advice\BeforeAdvice'];
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\MVC\Web\Routing\UriBuilder', 'build', $methodArguments);
				...
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['build']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['build']);
		}
	...
	$reflectedClass = new \ReflectionClass('TYPO3\FLOW3\MVC\Web\Routing\UriBuilder');
	...
	foreach($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyTaggedWith('TYPO3\FLOW3\MVC\Web\Routing\UriBuilder', $propertyName, 'transient')) continue;
	 ...
	 */
	 private function FLOW3_Proxy_injectProperties() {
		$this->injectRouter(\TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\MVC\Web\Routing\RouterInterface'));
		$this->injectEnvironment(\TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Utility\Environment'));
	}
}
#0             %CLASS%TYPO3_FLOW3_MVC_Web_Routing_UriBuilder7408      