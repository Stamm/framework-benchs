 ...
 * @scope singleton
 */
class CacheCommandController extends CacheCommandController_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		\TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\FLOW3\Command\CacheCommandController', $this);
		parent::__construct();
	 ...
	 */
	 public function emitWarmupCaches() {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['emitWarmupCaches'])) {
		$result = parent::emitWarmupCaches();

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['emitWarmupCaches'] = TRUE;
			try {
			
					$methodArguments = array();

					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Command\CacheCommandController', 'emitWarmupCaches', $methodArguments);
					$result = $this->FLOW3_AOP_Proxy_invokeJoinPoint($joinPoint);

					$advices = $this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices['emitWarmupCaches']['TYPO3\FLOW3\AOP\Advice\AfterReturningAdvice'];
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Command\CacheCommandController', 'emitWarmupCaches', $methodArguments, NULL, $result);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}

			} catch(\Exception $e) {
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['emitWarmupCaches']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['emitWarmupCaches']);
		}
	...
	$reflectedClass = new \ReflectionClass('TYPO3\FLOW3\Command\CacheCommandController');
	...
	foreach($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyTaggedWith('TYPO3\FLOW3\Command\CacheCommandController', $propertyName, 'transient')) continue;
	 ...
	 */
	 private function FLOW3_Proxy_injectProperties() {
		$this->injectCacheManager(\TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Cache\CacheManager'));
		$this->injectReflectionService(\TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Reflection\ReflectionService'));
	}
}
#0             %CLASS%TYPO3_FLOW3_Command_CacheCommandController %CLASS%TYPO3_FLOW3_MVC_Controller_CommandController7650      