 ...
 * @scope singleton
 */
class AccessDecisionVoterManager extends AccessDecisionVoterManager_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		\TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\FLOW3\Security\Authorization\AccessDecisionVoterManager', $this);
		\TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\FLOW3\Security\Authorization\AccessDecisionManagerInterface', $this);

		if (!isset($arguments[0])) $arguments[0] = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Object\ObjectManagerInterface');
		if (!isset($arguments[1])) $arguments[1] = \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->get('TYPO3\FLOW3\Security\Context');
		if (!isset($arguments[0])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $objectManager in class ' . __CLASS__ . '. Please check your calling code and Dependency Injection configuration.', 1296143787);
		if (!isset($arguments[1])) throw new \TYPO3\FLOW3\Object\Exception\UnresolvedDependenciesException('Missing required constructor argument $securityContext in class ' . __CLASS__ . '. Please check your calling code and Dependency Injection configuration.', 1296143787);
	 ...
	 private function FLOW3_AOP_Proxy_buildMethodsAndAdvicesArray() {
		...
		$this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices = array(
			'decideOnJoinPoint' => array(
				'TYPO3\FLOW3\AOP\Advice\AfterThrowingAdvice' => array(
					new \TYPO3\FLOW3\AOP\Advice\AfterThrowingAdvice('TYPO3\FLOW3\Security\Aspect\LoggingAspect', 'logJoinPointAccessDecisions', $objectManager, NULL),
				),
			),
		);
	}
	 ...
	 */
	 public function decideOnJoinPoint(\TYPO3\FLOW3\AOP\JoinPointInterface $joinPoint) {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['decideOnJoinPoint'])) {
		$result = parent::decideOnJoinPoint($joinPoint);

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['decideOnJoinPoint'] = TRUE;
			try {
		...
		try {
					...
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Security\Authorization\AccessDecisionVoterManager', 'decideOnJoinPoint', $methodArguments);
					$result = $this->FLOW3_AOP_Proxy_invokeJoinPoint($joinPoint);

			} catch (\Exception $exception) {

				$advices = $this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices['decideOnJoinPoint']['TYPO3\FLOW3\AOP\Advice\AfterThrowingAdvice'];
				$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\Security\Authorization\AccessDecisionVoterManager', 'decideOnJoinPoint', $methodArguments, NULL, NULL, $exception);
				...
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['decideOnJoinPoint']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['decideOnJoinPoint']);
		}
	...
	$reflectedClass = new \ReflectionClass('TYPO3\FLOW3\Security\Authorization\AccessDecisionVoterManager');
	...
	foreach($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyTaggedWith('TYPO3\FLOW3\Security\Authorization\AccessDecisionVoterManager', $propertyName, 'transient')) continue;
...
#0             %CLASS%TYPO3_FLOW3_Security_Authorization_AccessDecisionVoterManager9453      